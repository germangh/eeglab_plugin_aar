function [W, nonseparablecomponents, Wefica, Wwasobi, ISRwa1, ISRef1, signals]= multicombi(X,AR_order)
%
% Clustering based combination of EFICA and WASOBI
% version 1.0  (release 8.12.2006)
% Note: This version has been modified by Germán Gómez-Herrero in order to 
% use iWASOBI instead of eWASOBI.
...
[Wefica, ISRef1]=efica(x,eye(dim));
[Wwasobi,AOL_init,ISRwa1]= iwasobi(Wefica*x,AR_order,0.99);
...
while IsUnresolved(stack) %stop if there are only nonseparable components
    ...
    [Wef, ISRef] = efica(y,eye(size(y,1)));
    [Wwa,AOL_init, ISRwa] = iwasobi(y,AR_order,0.99);
